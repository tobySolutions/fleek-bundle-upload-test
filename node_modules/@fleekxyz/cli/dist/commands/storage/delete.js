...
const deleteStorageAction = async ({ sdk, args }) => {
    ...
    try {
        await (0, processPromisesBatch_1.processPromisesBatch)(cidsToDelete, async (cid) => {
            ...
            cli_1.output.log(`${cid ? `cid: ${cid}` : ''}${name ? ` name: ${name}` : ''}`);
            if (response.status === 200) {
                cli_1.output.success((0, translation_1.t)('commonItemActionSuccess', { subject: `${(0, translation_1.t)('storage')}`, action: (0, translation_1.t)('deleted') }));
            }
