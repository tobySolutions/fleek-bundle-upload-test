...
const getDeploymentWorkflowYamlLocation = async () => {
    const useCustomLocation = await (0, confirmWorkflowCustomPathPrompt_1.confirmWorkflowCustomPathPrompt)({ path: prepareGitHubActionsIntegration_1.ghActionsDeploySitesYamlPath });
    if (useCustomLocation === false) {
        return prepareGitHubActionsIntegration_1.ghActionsDeploySitesYamlPath;
    }
    const directory = await (0, enterDirectoryPathPrompt_1.enterDirectoryPathPrompt)({ message: (0, translation_1.t)('specifyDirToSaveWorkConf') });
    const yamlPath = (0, path_1.join)(directory, prepareGitHubActionsIntegration_1.ghWorkflowFilename);
    return yamlPath;
};
