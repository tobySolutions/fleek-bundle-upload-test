...
const getConfigurationPath_1 = require("./getConfigurationPath");
const readConfigurationFile = async ({ predefinedConfigPath, }) => {
    const configPath = await (0, getConfigurationPath_1.getConfigurationPath)({ predefinedConfigPath });
    const fileExtension = (0, path_1.extname)(configPath);
    if (fileExtension === '.json') {
        const content = await fs_1.promises.readFile(configPath, 'utf8').catch(() => Promise.reject(new errors_1.FleekConfigMissingFileError({})));
        try {
            return { configuration: JSON.parse(content), configPath };
        }
        catch (e) {
            throw new errors_1.FleekConfigInvalidContentError({ configPath, validationResult: (0, translation_1.t)('jsonNotValid') });
        }
    }
    if (fileExtension === '.ts') {
    ...
    throw new errors_1.FleekConfigInvalidContentError({ configPath, validationResult: (0, translation_1.t)('unknownFileExt') });
};
