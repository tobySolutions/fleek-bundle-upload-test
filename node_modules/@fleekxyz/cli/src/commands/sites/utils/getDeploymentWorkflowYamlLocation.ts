...
import { ghActionsDeploySitesYamlPath, ghWorkflowFilename } from './prepareGitHubActionsIntegration';

export const getDeploymentWorkflowYamlLocation = async (): Promise<string> => {
  const useCustomLocation = await confirmWorkflowCustomPathPrompt({ path: ghActionsDeploySitesYamlPath });

  if (useCustomLocation === false) {
    return ghActionsDeploySitesYamlPath;
  }

  const directory = await enterDirectoryPathPrompt({ message: t('specifyDirToSaveWorkConf') });
