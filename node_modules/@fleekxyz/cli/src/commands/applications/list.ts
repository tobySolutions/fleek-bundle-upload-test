...
export const listApplicationsAction: SdkGuardedFunction<Record<string, never>> = async ({ sdk }) => {
  ...
  output.table(
    applications.map(({ id, name, clientId, whitelistDomains, whiteLabelDomains, createdAt }) => {
...
export const listApplicationsActionHandler = withGuards(listApplicationsAction, {
  scopes: {
    authenticated: true,
    project: true,
    site: false,
